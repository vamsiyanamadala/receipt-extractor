...
A streaming receipt-extraction app. Drop in an image, watch Claude's vision model fill a structured form field-by-field, edit anything, submit. Built with Malaysian SST, service charge, MYR, and MyInvois e-invoice awareness — plus a real evaluation harness.

Submitted for the **Teleperformance Malaysia AI Intern Build Challenge**, May 2026.

**Live:** https://receipt-extractor-six.vercel.app
**Demo:** https://www.loom.com/share/cf20dc7d8246425d94f285071e723828
**Repo:** https://github.com/vamsiyanamadala/receipt-extractor
...
- **Strict structured outputs** via Claude tool-use (`tool_choice: "extract_receipt"`) — guaranteed-shape JSON, no parse failures.
...
- **Prompt caching** — system prompt is cached with `cache_control: ephemeral`.
...
| Metric | Result | Notes |
| --- | --- | --- |
| Field-level accuracy | **97.5%** (39/40) | merchant, date, total, currency, subtotal, sst_amount, service_charge |
...
A Starbucks Malaysia receipt labels its tax as "Service Tax (ST)" rather than "SST". The model captured the tax amount correctly (RM 1.47) but classified it ambiguously between the `sst_amount` and `service_charge` fields. This is a real ambiguity in post-2018 Malaysian receipt vocabulary — "Service Tax" *is* the S in SST, but establishments often still print it on its own line. Roadmap: tighter system-prompt disambiguation + a regex pre-classifier on common Malaysian tax label variants.

### A note on `/eval` in production

The `/eval` page in the live app times out on Vercel's Hobby tier (60-second function limit) because running 8 sequential extractions takes ~75 seconds. **Real numbers come from `npm run eval` locally.** Running `/eval` as a synchronous HTTP route was always a tradeoff — production-grade evals belong in CI, not in a serverless function. Roadmap fix is a job queue (Vercel Queues or Inngest) that runs eval async and writes results back to a KV store.

### Run it yourself

```bash
npm run eval         # CLI runner — this is the authoritative one
# or visit /eval in a locally-running app
```

The runner loads `.jpg`/`.png`/`.webp` images from `eval/fixtures/`, matches each to a `.json` golden label in `eval/golden/`, and reports per-field accuracy, full-record exact-match rate, and latency / cost.

**Honesty note**: when you publish accuracy numbers, share the failure modes. A 78% with documented errors beats an unbacked 99%.
...
| Structured output | Tool-use with `tool_choice` + strict input schema |
...
`claude-sonnet-4-6` (Anthropic's current default Sonnet, released 17 Feb 2026) leads independent vision benchmarks for document/OCR. At $3/$15 per MTok with 1M-token context, it's the right balance for an audience that values document accuracy.

Considered and rejected:

- **Claude Opus 4.7** — better, but ~5× cost. Worth it for adversarial cases; plan is to route only those to Opus.
...
│ Next.js  │ ◄──────────────────── │   Node runtime      │ ◄── input_json   ──── │  Sonnet 4.6  │
...
2. The route validates type / size, base64-encodes, and calls Anthropic with:
    ...
    - Forced tool use (`tool_choice: { type: "tool", name: "extract_receipt" }`)
    - Strict input schema (`additionalProperties: false`, required fields)
3. The stream is parsed: each `input_json_delta` event is appended; `partial-json` parses the accumulated string and yields a `partial` SSE event to the client whenever it changes.
...
5. On `complete`, Zod re-validates the final object; cost is computed from `usage.input_tokens / output_tokens / cache_read_input_tokens / cache_creation_input_tokens` using known May-2026 pricing.
...
- **Authoritative on Malaysian rules** — SST replaced GST on 1 Sep 2018, service charge is separate from tax, IRBM UIN is 15 chars, MyInvois Phase 4 requires it for transactions > RM 10,000 from 1 Jan 2026.
- **Tool-grounded** — instructs the model that output is delivered exclusively via the `extract_receipt` tool call.
...
```bash
git clone https://github.com/vamsiyanamadala/receipt-extractor
...
│   ├── fixtures/                  # 8 hand-verified test receipts
...
- **Fraud / AI-generated-receipt detection** — needs EXIF, pixel-level forensics, and a copy-move detector; out of scope for a one-week build, but the pipeline is ready for it.
